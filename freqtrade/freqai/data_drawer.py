...
class FreqaiDataDrawer:
    ...
    def load_metric_update_tracker_from_disk(self):
        ...
        if exists:
            with Path.open(self.metric_update_tracker_path, "r") as fp:
                self.metric_update_tracker = rapidjson.load(fp, number_mode=rapidjson.NM_NATIVE)
                for key in self.metric_update_tracker:
                    self.metric_update_tracker[key]["datetime_updated"] = datetime.fromtimestamp(
                        self.metric_update_tracker[key]["datetime_updated"], tz=timezone.utc)
            logger.info("Loading existing metric update tracker (external data) from disk.")
            with Path.open(self.metric_slug_path, "r") as fp:
            ...
            logger.info(
                "Could not find existing metric update tracker or"
                "slug list (external data), starting from scratch")
    ...
    def load_historic_external_data_from_disk(self):
        ...
        if exists:
            ...
            with Path.open(self.historic_external_data_path, "rb") as fp:
    ...
    def save_historic_external_data_to_disk(self):
        ...
        """
        with Path.open(self.historic_external_data_path, "wb") as fp:
    ...
    def save_metric_update_tracker_to_disk(self):
        ...
        with self.save_lock:
            ...
            for key in self.metric_update_tracker:
                serializable[key]["datetime_updated"] = serializable[key][
                                                            "datetime_updated"].timestamp()
            with Path.open(self.metric_update_tracker_path, 'w') as fp:
                rapidjson.dump(serializable, fp, default=self.np_encoder,
                               number_mode=rapidjson.NM_NATIVE)
            with Path.open(self.metric_slug_path, 'w') as fp:
